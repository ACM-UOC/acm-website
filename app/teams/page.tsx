...
export default function TeamsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 relative z-0">
      ...
      >
        {/* Page Hero Header */}
        <motion.div variants={itemVariants} className="text-center max-w-3xl mx-auto mb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {teamIds.map((id) => (
            <Link href={`/teams/${id}`} key={id} className="block group outline-none">
              <motion.div 
                variants={itemVariants}
                className="relative h-full bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden flex flex-col"
              >
